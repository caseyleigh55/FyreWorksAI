...
internal static class JobFinancialBuilder
{
    ...
    private static List<ScheduleValueItem> BuildBaseScheduleOfValues(BaselineEstimate baseline)
    {
        ...
        AddScheduleItem(items, JobCostCodes.Admin, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Admin), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Admin));
        AddScheduleItem(items, JobCostCodes.Engineering, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Engineering), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Engineering));
        AddScheduleItem(items, JobCostCodes.Materials, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Materials), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Materials));
        AddScheduleItem(items, JobCostCodes.Install, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Install), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Install));
        AddScheduleItem(items, JobCostCodes.Demo, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Demo), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Demo));
        AddScheduleItem(items, JobCostCodes.Trim, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Trim), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Trim));
        AddScheduleItem(items, JobCostCodes.Test, JobFinancialMath.GetBaselineBidSaleReference(baseline, JobCostCodes.Test), JobFinancialMath.GetBaselineScheduledRevenue(baseline, JobCostCodes.Test));
        return items;
    }

    private static void AddScheduleItem(List<ScheduleValueItem> items, string categoryCode, decimal referenceAmount, decimal scheduledAmount)
    {
        referenceAmount = EstimateMath.RoundCurrency(Math.Max(0m, referenceAmount));
        scheduledAmount = EstimateMath.RoundCurrency(Math.Max(0m, scheduledAmount));
        if (referenceAmount <= 0m && scheduledAmount <= 0m)
        {
            return;
        }
        ...
        items.Add(new ScheduleValueItem
        {
            ...
            ScheduledValue = scheduledAmount,
    ...
    private static void EnsureScheduleValueChildCollections(JobRecord job)
    {
        foreach (var item in job.ScheduleOfValues)
        {
            ...
            if (item.SubLines.Count == 0 && (hasLegacyProgressValues || shouldCreateEstimatedSaleSubLine))
            {
                ...
                {
                    Description = shouldCreateEstimatedSaleSubLine ? "Bid Sale Reference" : $"{item.Description} Progress",
                    LineValue = shouldCreateEstimatedSaleSubLine
                        ? GetScheduleValueDefaultSubLineValue(item)
    ...
    private static void SyncBaseScheduleValues(JobRecord job)
    {
        ...
        {
            var baselineReferenceAmount = JobFinancialMath.GetBaselineBidSaleReference(job.Baseline, categoryCode);
            var baselineScheduledAmount = JobFinancialMath.GetBaselineScheduledRevenue(job.Baseline, categoryCode);
            ...
            if (baselineReferenceAmount <= 0m && baselineScheduledAmount <= 0m)
            {
            ...
            var shouldSyncScheduledAllocation = ShouldSyncAutoGeneratedScheduleAllocation(existingAutoGenerated, baselineScheduledAmount);
            existingAutoGenerated.ReferenceValue = baselineReferenceAmount;
            if (shouldSyncScheduledAllocation)
            {
                ApplyAutoGeneratedScheduleAllocation(existingAutoGenerated, baselineReferenceAmount);
            }
        }
    }
    ...
    private static void MigrateLegacyAdminEngineeringScheduleValues(JobRecord job)
    {
        ...
        var adminReferenceAmount = JobFinancialMath.GetBaselineBidSaleReference(job.Baseline, JobCostCodes.Admin);
        var engineeringReferenceAmount = JobFinancialMath.GetBaselineBidSaleReference(job.Baseline, JobCostCodes.Engineering);
    ...
    private static bool ShouldSyncAutoGeneratedScheduleAllocation(ScheduleValueItem item, decimal previousDefaultAmount)
    {
        if (!item.IsAutoGenerated || item.IsChangeOrderLine)
        {
            return false;
        }
        ...
        return item.SubLines.Count == 1 &&
               item.SubLines[0].LinkedCommitmentId is null &&
               (item.SubLines[0].IsAutoGenerated ||
                AreEquivalentCurrencyAmounts(item.SubLines[0].LineValue, previousDefaultAmount)) &&
               AreEquivalentCurrencyAmounts(item.SubLines[0].LineValue, item.ScheduledValue);
    }

    private static void ApplyAutoGeneratedScheduleAllocation(ScheduleValueItem item, decimal scheduledAmount)
    {
        item.ScheduledValue = EstimateMath.RoundCurrency(Math.Max(0m, scheduledAmount));
        if (item.SubLines.Count == 1 &&
            item.SubLines[0].LinkedCommitmentId is null)
        {
            item.SubLines[0].LineValue = item.ScheduledValue;
        }
    }

    private static bool AreEquivalentCurrencyAmounts(decimal left, decimal right) =>
        Math.Abs(EstimateMath.RoundCurrency(left) - EstimateMath.RoundCurrency(right)) < 0.01m;
    ...
    private static decimal GetScheduleValueDefaultSubLineValue(ScheduleValueItem item) =>
